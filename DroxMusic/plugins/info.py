...
@DroxMusic.on(events.NewMessage(pattern="^[!?/]الايدي"))
async def id(event):
...
@DroxMusic.on(events.NewMessage(pattern="^[!?/]ايدي ?(.*)"))
async def info(event):

    sed = await DroxMusic(P(user_id=event.sender_id, offset=42, max_id=0, limit=80))
    hn = await DroxMusic(GetFullUserRequest(event.sender_id))
    ...
    if not input_str:
          await DroxMusic.send_message(event.chat_id, text.format(hn.user.first_name, hn.user.last_name, event.sender_id, event.sender.username, sed.count, hn.about, event.sender_id))
          return
 
    input_str = event.pattern_match.group(1)
    ha = await DroxMusic.get_entity(input_str)
    hu = await DroxMusic(GetFullUserRequest(id=input_str))
    sedd = await DroxMusic(P(user_id=input_str, offset=42, max_id=0, limit=80))
...
@DroxMusic.on(events.callbackquery.CallbackQuery(data="misc"))
async def _(event):
