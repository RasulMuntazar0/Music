...
@DroxMusic.on(events.NewMessage(pattern="^[?!/]الرسائل المثبتة"))
async def get_pinned(event):
    chat_id = (str(event.chat_id)).replace("-100", "")

    Ok = await DroxMusic.get_messages(event.chat_id, ids=types.InputMessagePinned()) 
...
@DroxMusic.on(events.NewMessage(pattern="^[!?/]تثبيت ?(.*)"))
@is_admin
async def pin(event, perm):
    ...
    if "notify" in input_str:
       await DroxMusic.pin_message(event.chat_id, msg, notify=True)
       return
    await DroxMusic.pin_message(event.chat_id, msg)   

@DroxMusic.on(events.NewMessage(pattern="^[!?/]الغاء تثبيت ?(.*)"))
@is_admin
...
@DroxMusic.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
