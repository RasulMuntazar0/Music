...
@DroxMusic.on(events.callbackquery.CallbackQuery(data="admin"))
async def _(event):

    await event.edit(ADMIN_TEXT, buttons=[[Button.inline("« رجوع", data="help")]])

@DroxMusic.on(events.callbackquery.CallbackQuery(data="play"))
async def _(event):

    await event.edit(PLAY_TEXT, buttons=[[Button.inline("« رجوع", data="help")]])

@DroxMusic.on(events.NewMessage(pattern="^[!?/]رفع ?(.*)"))
@is_admin
async def promote(event, perm):
    ...
    if not input_str and not user:
        ...
        return
    sed = await DroxMusic(GetFullUserRequest(id=user.sender_id or input_str))
    await DroxMusic(EditAdminRequest(event.chat_id, user.sender_id or input_str, ChatAdminRights(
                    add_admins=False,
                    invite_users=True,
                    change_info=False,
                    ban_users=True,
...
@DroxMusic.on(events.NewMessage(pattern="^[!?/]تنزيل ?(.*)"))
@is_admin
async def promote(event, perm):
    if event.is_private:
       await event.reply("يستخدم هذا الامر فقط في المجموعات")
...
@DroxMusic.on(events.NewMessage(pattern="^[!?/]الرابط"))
async def invitelink(event):

    if event.is_private:
       await event.reply("يستخدم هذا الامر فقط في المجموعات !")
       return
    link = await DroxMusic(ExportChatInviteRequest(event.chat_id))
